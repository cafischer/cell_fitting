...
if __name__ == '__main__':
    ...
    # simulate cell
    v_model, t_model, i_inj = simulate_model(cell, 'rampIV', ramp_amp, tstop=tstop, v_init=v_init, onset=200, dt=dt)
    currents, channel_list = simulate_model_currents(cell, 'rampIV', ramp_amp,
                                                     tstop=tstop, v_init=v_init, onset=200, dt=dt)
    ...
    for percent_block_idx, percent_block in enumerate(percent_blocks):
        ...
        for i, channel_name in enumerate(channel_list):
            ...
            v_after_block[i, :], _, _ = simulate_model(cell, 'rampIV', ramp_amp,
                                                       tstop=tstop, v_init=v_init, onset=200, dt=dt)
    ...
    for percent_block_idx, percent_block in enumerate(percent_blocks):
        ...
        for i, channel_name in enumerate(channel_list):
            cell = Cell.from_modeldir(os.path.join(save_dir_model, model, 'cell.json'))
            block_channel_at_timepoint(cell, channel_name, percent_block, t_model[fAHP_min_idx]+onset)
            v_after_block[i, :], _, _ = simulate_model(cell, 'rampIV', ramp_amp,
                                                       tstop=tstop, v_init=v_init, onset=onset, dt=dt)
