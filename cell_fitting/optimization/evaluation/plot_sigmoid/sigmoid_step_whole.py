...
from cell_fitting.optimization.evaluation.plot_sigmoid import sig
...
if __name__ == '__main__':
    ...
    currents = []
    for a in a_range:
        i_inj = np.concatenate((sig(0, 1, a, x), np.ones(to_idx(offset_dur, dt))))
    ...
    # plot
    save_img = os.path.join(save_dir, 'img', 'plot_IV', 'sigmoid_step', 'whole')
    ...
    # pl.figure()
    # for i, (i_inj, t) in enumerate(zip(i_inj_traces, t_traces)):
    #     pl.plot(t[:len(i_inj)], i_inj, label='Slope: '+str(a_range[i]))
    # pl.xlabel('Time (ms)')
