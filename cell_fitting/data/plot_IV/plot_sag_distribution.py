...
if __name__ == '__main__':
    ...
    v_shift = -16
    animal = 'rat'
    save_dir_img = os.path.join('../plots/plot_IV/sag_hist', animal)
    amp = -0.15
    ...
    sweep_idx = get_sweep_index_for_amp(amp, 'plot_IV')
    ...
    for cell_id in cells:
        ...
        # read data
        v_mat_data, t_mat_data = get_v_and_t_from_heka(os.path.join(data_dir, cell_id + '.dat'), 'plot_IV',
                                                                   sweep_idxs=[sweep_idx])
        i_inj_mat = get_i_inj_from_function('plot_IV', [sweep_idx], t_mat_data[0][-1], t_mat_data[0][1]-t_mat_data[0][0])
