...
if __name__ == '__main__':
    ...
    for model_idx, model in enumerate(models):
        ...
        for ramp3_times_idx in range(len(ramp3_times)):
            ax1.plot(t_exp, i_inj_mat[np.where(np.isclose(ramp3_amp_exp + ramp3_amp_idx_exp * 0.05,
                                                          double_ramp_params['ramp3_amps']))[0][0],
                            ramp3_times_idx, :, 1], color_exp)
            ax1.plot(t_exp, i_inj_mat[np.where(np.isclose(ramp3_amp_model + ramp3_amp_idx_model * 0.05,
                                                          double_ramp_params['ramp3_amps']))[0][0],
        ...
        ax0 = pl.Subplot(fig, outer[1, model_idx])
        fig.add_subplot(ax0)
        ...
        plot_current_threshold_on_ax(ax0, colors_dict={-0.1: color_model, 0.0: color_model, 0.1: color_model},
                                     ...
                                     legend_loc=None, **current_threshold_dict_model)
    ...
    pl.tight_layout()
    pl.subplots_adjust(top=0.97, bottom=0.06, left=0.07, right=0.99)
    pl.savefig(os.path.join(save_dir_img, 'reproduction_double_ramp_models.png'))
    pl.show()