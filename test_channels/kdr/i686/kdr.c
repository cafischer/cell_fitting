...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 10, 4);
 	...
 	ivoc_help("help ?1 kdr /media/caro/Daten/Phd/DAP-Project/cell_fitting/test_channels/Kdr/i686/kdr.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static char *modelname = "delayed recitifier potassium current";
...
static int  rates ( _threadargsprotocomma_ double _lv ) {
   double _la , _ltadj ;
 _ltadj = pow( q10 , ( ( celsius - temp ) / 10.0 ) ) ;
   _la = alpn ( _threadargscomma_ _lv ) ;
   ninf = 1.0 / ( 1.0 + _la ) ;
   taun = betn ( _threadargscomma_ _lv ) / ( _ltadj * a0n * ( 1.0 + _la ) ) ;
   if ( taun < nmin ) {
     taun = nmin ;
     }
   taun = taun / nscale ;
