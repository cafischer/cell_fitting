...
if __name__ == '__main__':
    ...
    t_AP = t_after+t_before
    idx_before = int(round(t_before / dt))
    idx_after = int(round(t_after / dt))
    v_APs_removed = copy.copy(v)
    for onset in AP_onsets:
        slope = (v[onset-idx_before] - v[onset+idx_after]) / (0 - t_AP)
        v_APs_removed[onset-idx_before:onset+idx_after+1] = slope * np.arange(0, t_AP+dt/2, dt) + v[onset-idx_before]
