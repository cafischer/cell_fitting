...
#define hinf _p[7]
#define htau _p[8]
#define tadj_act _p[9]
#define tadj_inact _p[10]
#define Dm _p[11]
#define Dh _p[12]
#define v _p[13]
#define _g _p[14]
...
#define celsius_channel celsius_channel_Nap
 double celsius_channel = 22;
#define q10_inact q10_inact_Nap
 double q10_inact = 1.3;
#define q10_act q10_act_Nap
 double q10_act = 2.7;
#define usetable usetable_Nap
 ...
 "q10_act_Nap", &q10_act_Nap,
 "q10_inact_Nap", &q10_inact_Nap,
 "celsius_channel_Nap", &celsius_channel_Nap,
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 15, 4);
 	...
 	ivoc_help("help ?1 Nap /media/caro/Daten/Phd/DAP-Project/cell_fitting/model/channels/i686/INap.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   ...
   tadj_act = pow( q10_act , ( ( celsius - celsius_channel ) / 10.0 ) ) ;
   tadj_inact = pow( q10_inact , ( ( celsius - celsius_channel ) / 10.0 ) ) ;
   Dm = ( minf - m ) * tadj_act / mtau ;
   Dh = ( hinf - h ) * tadj_inact / htau ;
   }
 return _reset;
}
 ...
 tadj_act = pow( q10_act , ( ( celsius - celsius_channel ) / 10.0 ) ) ;
 tadj_inact = pow( q10_inact , ( ( celsius - celsius_channel ) / 10.0 ) ) ;
 Dm = Dm  / (1. - dt*( ( (( ( - 1.0 ) ))*(tadj_act) ) / mtau )) ;
 Dh = Dh  / (1. - dt*( ( (( ( - 1.0 ) ))*(tadj_inact) ) / htau )) ;
 return 0;
}
 /*END CVODE*/
 static int states (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
   ...
   tadj_act = pow( q10_act , ( ( celsius - celsius_channel ) / 10.0 ) ) ;
   tadj_inact = pow( q10_inact , ( ( celsius - celsius_channel ) / 10.0 ) ) ;
    m = m + (1. - exp(dt*(( (( ( - 1.0 ) ))*(tadj_act) ) / mtau)))*(- ( ( (( minf ))*(tadj_act) ) / mtau ) / ( ( (( ( - 1.0) ))*(tadj_act) ) / mtau ) - m) ;
    h = h + (1. - exp(dt*(( (( ( - 1.0 ) ))*(tadj_inact) ) / htau)))*(- ( ( (( hinf ))*(tadj_inact) ) / htau ) / ( ( (( ( - 1.0) ))*(tadj_inact) ) / htau ) - h) ;
   }
  return 0;
}
...
static int  _f_settables ( _threadargsprotocomma_ double _lv ) {
   minf = 1.0 / ( 1.0 + exp ( - ( _lv + 48.7 ) / 4.4 ) ) ;
   if ( _lv  == - 38.0 ) {
     mtau = .0013071895424837 ;
     }
   else {
     mtau = 1.0 / ( ( .091 * 1000.0 * ( _lv + 38.0 ) ) / ( 1.0 - exp ( - ( _lv + 38.0 ) / 5.0 ) ) + ( - .062 * 1000.0 * ( _lv + 38.0 ) ) / ( 1.0 - exp ( ( _lv + 38.0 ) / 5.0 ) ) ) ;
     }
   hinf = 1.0 / ( 1.0 + exp ( ( _lv + 48.8 ) / 9.98 ) ) ;
   htau = 1.0 / ( ( - 2.88 * .001 * ( _lv + 17.049 ) ) / ( 1.0 - exp ( ( _lv - 49.1 ) / 4.63 ) ) + ( 6.94 * .001 * ( _lv + 64.409 ) ) / ( 1.0 - exp ( - ( _lv + 447.0 ) / 2.63 ) ) ) ;
    return 0; }
