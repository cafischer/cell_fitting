...
if __name__ == '__main__':
    ...
    d_amp = 0.1
    amp2s = np.arange(0.1, 2.0+d_amp, d_amp)
    d_freq = 5
    freq2s = np.arange(5, 25+d_freq, d_freq)

    ISI_first = init_nan((len(amp2s), len(freq2s)))

    save_dir_img = os.path.join(save_dir, 'img', 'plot_sine_stimulus', 'when_doublet', 'middle')
    ...
    for i, amp2 in enumerate(amp2s):
        for j, freq2 in enumerate(freq2s):
            ...
            len_period = 1 / freq2 * 1000
            n_periods = sine1_dur / len_period
            n_periods_half = np.floor(n_periods/2)
            idx_start = to_idx(np.round(n_periods_half * len_period + onset_dur, 2), dt)
            idx_end = to_idx(np.round(n_periods_half * len_period + onset_dur + len_period, 2), dt)
            onsets = get_AP_onset_idxs(v[idx_start:idx_end], AP_threshold)  # use only period in the middle
            if len(onsets) >= 2:
            ...
            pl.plot(t, v, 'k')
            pl.plot(t[idx_start:idx_end], v[idx_start:idx_end], 'r')
            pl.xlabel('Time (ms)')
