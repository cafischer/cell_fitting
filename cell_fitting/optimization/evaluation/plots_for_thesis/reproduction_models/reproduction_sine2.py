...
if __name__ == '__main__':
    ...
    for model_idx, model in enumerate(models):
        ...
        if model_idx == 0:
            ...
            ax0.get_yaxis().set_label_coords(-0.23, 0.2)
            ax2.get_yaxis().set_label_coords(-0.23, 0.9)
            ax0.legend()
            ax1.legend()
        ...
        if model_idx == 0:
            ...
            ax0.get_yaxis().set_label_coords(-0.23, 0.5)
            ax1.get_yaxis().set_label_coords(-0.23, 0.5)
            ax0.text(-0.4, 1.0, 'B', transform=ax0.transAxes, size=18, weight='bold')

        # time vs phase
        ax = pl.Subplot(fig, outer[2, model_idx])
        fig.add_subplot(ax)
        ax.plot(np.array(sine_dict_model['t_phases']) / 1000., sine_dict_model['phases'], marker='o',
                color=color_model, linestyle='', alpha=0.5)
        ax.plot(np.array(sine_dict_data['t_phases']) / 1000., sine_dict_data['phases'], marker='o',
                color=color_exp, linestyle='', alpha=0.8)
        ...
        if model_idx == 0:
            ...
            ax.get_yaxis().set_label_coords(-0.23, 0.5)
    ...
    pl.tight_layout()
    pl.subplots_adjust(bottom=0.06, top=0.97, right=0.99, left=0.06, hspace=0.27)
