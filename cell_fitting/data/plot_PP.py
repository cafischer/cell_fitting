...
for c_idx, cell in enumerate(cells):
    ...
    for step_flag in step_flags:
        ...
        ramp3_amp_idxs = max(1, int(np.floor(len(protocols) / 30.0)))

        v_mat = np.zeros((ramp3_amp_idxs, len_ramp3_times, len_t))
        for ramp3_amp_idx in range(ramp3_amp_idxs):
            start = int((ramp3_amp_idx * 30) + step_flag + offset[c_idx])
            end = int(min(len(protocols), (((ramp3_amp_idx + 1) * 30) - 2) + step_flag + offset[c_idx]))
